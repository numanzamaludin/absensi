...
class JadwalModel
{
    ...
    {
        $hari = $this->ubahHariKeIndonesia(date('l')); // Ubah dari "Monday" ke "Senin", dst
        $waktu = date('H:i:s');
        ...
        $stmt = $this->db->prepare("
            ...
            WHERE gm.id_guru = :id_guru
            AND j.hari = :hari
            AND :waktu BETWEEN j.jam_mulai AND j.jam_selesai
            LIMIT 1
        ");
        $stmt->execute([
            'id_guru' => $id_guru,
    ...
    private function ubahHariKeIndonesia($day)
    {
        $days = [
            'Sunday' => 'Minggu',
            'Monday' => 'Senin',
    ...
    // {
    //     $stmt = $this->db->prepare("
    //         SELECT j.*, g.id_guru, g.nama_guru, m.nama_mapel, k.nama_kelas
    //         FROM jadwal j
    //         JOIN guru_mapel gm ON j.id_guru_mapel = gm.id_guru_mapel
