...
class AbsensiModel
{
    ...
    public function getJadwalAktifSekarang($id_guru)
    {
        $hariMap = [
            'Monday' => 'Senin',
            'Tuesday' => 'Selasa',
        ...
        $sql = "SELECT j.* 
            FROM jadwal j
            JOIN guru_mapel gm ON gm.id_guru_mapel = j.id_guru_mapel
            WHERE gm.id_guru = ? 
              AND j.hari = ? 
        ...
        $stmt = $this->db->prepare($sql);
        $stmt->execute([$id_guru, $hari, $waktu]);
        return $stmt->fetch(PDO::FETCH_ASSOC);
    }

    public function cekSudahAbsenGuru($id_guru, $id_jadwal)
    {
        $sql = "SELECT COUNT(*) 
            FROM absensi_guru
            WHERE id_guru = ? 
    ...
    public function simpanAbsenGuru($id_guru, $id_jadwal, $lokasi)
    {
        $sql = "INSERT INTO absensi_guru (id_guru, id_jadwal, tanggal, waktu, lokasi, metode)
            VALUES (?, ?, CURDATE(), CURTIME(), ?, 'nearby')";

        $stmt = $this->db->prepare($sql);
        $stmt->execute([$id_guru, $id_jadwal, $lokasi]);
    }
    ...
    public function getAbsensiHarianGuru($idGuru, $tanggal)
    {
        $stmt = $this->db->prepare("
        SELECT a.*, k.nama_kelas, mp.nama_mapel, j.jam_mulai, j.jam_selesai
        FROM absensi_guru a
        JOIN jadwal j ON a.id_jadwal = j.id_jadwal
        JOIN guru_mapel gm ON j.id_guru_mapel = gm.id_guru_mapel
        JOIN mata_pelajaran mp ON gm.id_mapel = mp.id_mapel
        JOIN kelas k ON gm.id_kelas = k.id_kelas
        WHERE a.id_guru = ? AND a.tanggal = ?
    ");
    ...
    public function getAbsensiBulananGuru($idGuru, $bulan, $tahun)
    {
        $stmt = $this->db->prepare("
        SELECT a.*, k.nama_kelas, mp.nama_mapel, j.jam_mulai, j.jam_selesai
        FROM absensi_guru a
        ...
        JOIN mata_pelajaran mp ON gm.id_mapel = mp.id_mapel
        JOIN kelas k ON gm.id_kelas = k.id_kelas
        WHERE a.id_guru = ? AND MONTH(a.tanggal) = ? AND YEAR(a.tanggal) = ?
    ");
    ...
    public function getAbsensiHariIniAdmin($tanggal)
    {
        $stmt = $this->db->prepare("
